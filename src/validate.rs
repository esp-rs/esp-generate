...
/// Warn when a template declares an older `sdk_version` than the predicates it
/// uses require. Counts only SDK predicates, and only when every combination
/// rendered — a failed one evaluated an unknown share of the template.
