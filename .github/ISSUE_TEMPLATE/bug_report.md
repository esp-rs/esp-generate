---
name: Bug report
about: Create a report to help us improve
title: ''
labels: ["bug", "bug jury"]
assignees: ''

---

## Bug description

<!-- A clear and concise description of what the bug is. -->

## To Reproduce

<!-- Steps to reproduce the behavior. Note that after running the `esp-generate` command, you get the selected options in the terminal. -->
1. ...
2. ...

<!-- Make sure you are able to reproduce the bug in the `main` branch, too. -->

## Expected behavior

<!-- A clear and concise description of what you expected to happen. Attach screenshots if needed. -->

## Environment

- Target device: [e.g. ESP32-S3] <!-- Use `espflash board-info` to get the target device iformation. -->
- Crate version: [e.g. esp-generate 1.0.0]
